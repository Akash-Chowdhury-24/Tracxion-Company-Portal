...
const CommonFileUpload = ({
  ...
  const removeFile = (index) => {
    ...
    setFiles(newFiles);
    // For single file mode, pass null if no files, otherwise the file object
    if (multiple) {
      onFilesChange && onFilesChange(newFiles);
    } else {
      onFilesChange && onFilesChange(newFiles.length > 0 ? newFiles[0] : null);
    }
  };
  ...
  const handleReupload = () => {
    setFiles([]);
    setError('');
    // For single file mode, pass null, for multiple pass empty array
    onFilesChange && onFilesChange(multiple ? [] : null);
  };
  ...
  const styles = {
    ...
    uploadArea: {
      border: `2px solid ${isDragOver ? '#00A1F9' : files.length > 0 ? '#00A1F9' : '#00A1F9'}`,
      borderRadius: '12px',
      padding: '40px 20px',
      textAlign: 'center',
